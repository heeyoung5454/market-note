...
export default function ItemDetail({ code, name }: ItemDetailProps) {
  const { data, isLoading, error } = useDailyChart(code, 90);
      ...
      <div className='stock-detail-page__inner'>
        ...
        <section className='stock-detail__summary'>
          <div>
            ...
            <p className={`stock-detail__price ${change.className}`}>{isLoading ? "-" : `${Number(price).toLocaleString()}원`}</p>
          </div>
          <div>
            <p className='stock-detail__label'>등락률</p>
            <p className={`stock-detail__change ${change.className}`}>{isLoading ? "-" : change.text}</p>
          </div>
          {summary?.marketCap && (
            <div>
              <p className='stock-detail__label'>시가총액</p>
              <p className='stock-detail__value'>{Number(summary.marketCap).toLocaleString()}억</p>
            </div>
          )}
        </section>
        ...
        <section className='stock-detail__chart'>
          <div className='stock-detail__chart-header'>
            <p className='stock-detail__chart-title'>일별 추이 (3달)</p>
          </div>
          ...
          {error && <div className='stock-detail__chart-state stock-detail__chart-state--error'>차트 조회에 실패했습니다.</div>}

          {!isLoading && !error && data?.output && <DailyChart data={data.output} />}
        </section>
      </div>
    </main>
  );
