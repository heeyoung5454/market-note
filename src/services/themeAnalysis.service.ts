...
function toStockSnapshots(stocks: RankStock[]): MarketStockSnapshot[] {
  return stocks.slice(0, STOCK_LIMIT).map((stock) => ({
    ...
    metric: stock.amount
      ? `거래대금 ${Math.round(Number(stock.amount) / 100_000_000).toLocaleString()}억`
      : undefined,
  }));
}
...
export function buildThemeAnalysisPrompt(input: ThemeAnalysisInput) {
...
export async function collectThemeAnalysisInput(): Promise<ThemeAnalysisInput> {
  ...
  const [tradingAmountRaw, riseRaw, newsRaw] = await Promise.all([
  ...
  return {
    ...
    riseTop: toStockSnapshots(riseStocks).map((stock) => ({
      ...stock,
      metric: stock.changeRate ? `등락률 ${stock.changeRate}%` : undefined,
    })),
    tradingAmountTop: toStockSnapshots(tradingStocks),
  };
}
